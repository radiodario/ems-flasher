...
int main(int argc, char **argv) {
    ...
    // read the ROM header
        ...
        if (buf[HEADER_CGBFLAG] == (char)0x80 && buf[HEADER_SGBFLAG] == (char)0x03) {
            printf("CGB enhanced, SGB enhanced, DMG compatible\n");
        } else if (buf[HEADER_CGBFLAG] == (char)0x80) {
            printf("CGB enhanced, DMG compatible\n");
        } else if (buf[HEADER_CGBFLAG] == (char)0xC0) {
            printf("CGB only\n");
        } else if (buf[HEADER_CGBFLAG] == (char)0xC0 && buf[HEADER_SGBFLAG] == (char)0x03) {
            printf("CGB only, SGB enhanced (not a real set of options)\n");
        } else if (buf[HEADER_SGBFLAG] == (char)0x03) {
        ...
        if (buf[HEADER_SGBFLAG] == (char)0x03 && buf[HEADER_OLDLICENSEE] != (char)0x33) {
            printf("SGB functions were enabled, but Old Licensee field is not set to 33h. This game will not be able to use SGB functions on real hardware.\n");
        }
